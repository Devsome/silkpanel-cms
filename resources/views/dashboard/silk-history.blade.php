<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">
            {{ __('dashboard.silk_history') }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">

            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('dashboard') }}"
                    class="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                    {{ __('dashboard.back_to_dashboard') }}
                </a>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4">
                    {{ __('dashboard.silk_history') }}
                </h3>

                @if ($history->isEmpty())
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('dashboard.silk_history_empty') }}</p>
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            <thead>
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    @if ($isro)
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.item_name') }}</th>
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.changed_silk') }}</th>
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.remained_silk') }}</th>
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.silk_type') }}</th>
                                        <th class="pb-2 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.change_date') }}</th>
                                    @else
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.silk_offset') }}</th>
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.buy_quantity') }}</th>
                                        <th class="pb-2 pr-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.silk_remain') }}</th>
                                        <th class="pb-2 text-xs font-medium text-gray-500 dark:text-gray-400">
                                            {{ __('silk-history.table.auth_date') }}</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @php $historyItems = $history->items(); @endphp
                                @foreach ($historyItems as $i => $entry)
                                    @php
                                        $nextEntry = $historyItems[$i + 1] ?? null;
                                        $isSilkCharged =
                                            $entry->ChangedSilk == 0 &&
                                            $nextEntry !== null &&
                                            $entry->RemainedSilk > $nextEntry->RemainedSilk;
                                    @endphp
                                    <tr>
                                        @if ($isro)
                                            <td class="py-2 pr-4 font-medium text-gray-700 dark:text-gray-300">
                                                @php
                                                    $cpItemCode = $entry->CPItemCode ?? null;
                                                    $cpItemName = $entry->CPItemName ?? null;
                                                    $imgPath = $cpItemCode
                                                        ? public_path('images/silkroad/webmall/' . $cpItemCode . '.jpg')
                                                        : null;
                                                @endphp
                                                @if ($entry->PTInvoiceID)
                                                    <span class="flex items-center gap-2">
                                                        @if ($cpItemCode && $imgPath && \Illuminate\Support\Facades\File::exists($imgPath))
                                                            <img src="{{ asset('images/silkroad/webmall/' . $cpItemCode . '.jpg') }}"
                                                                alt="" width="32" height="32"
                                                                class="">
                                                        @endif
                                                        {{ $cpItemName }}
                                                    </span>
                                                @elseif($entry->ChangedSilk == 0 && $entry->RemainedSilk > 0)
                                                    <span
                                                        class="text-emerald-600 dark:text-emerald-400 font-bold">{{ __('silk-history.table.add_silk') }}</span>
                                                @endif
                                            </td>
                                            <td
                                                class="py-2 pr-4 font-medium
                                                {{ $entry->ChangedSilk == 0 || $isSilkCharged ? 'text-emerald-600 dark:text-emerald-400' : ($entry->ChangedSilk < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white') }}">
                                                @if ($entry->ChangedSilk == 0 && $entry->RemainedSilk > 0)
                                                    {{ number_format($entry->RemainedSilk) ?? number_format($entry->Silk_Offset) }}
                                                @else
                                                    {{ number_format($entry->ChangedSilk) }}
                                                @endif
                                            </td>
                                            <td class="py-2 pr-4 text-gray-700 dark:text-gray-300">
                                                {{ number_format($entry->RemainedSilk) }}</td>
                                            <td class="py-2 pr-4 text-gray-500 dark:text-gray-400">
                                                {{ \App\Enums\SilkTypeIsroEnum::tryFrom((int) $entry->SilkType)?->getLabel() ?? $entry->SilkType }}
                                            </td>
                                            <td class="py-2 text-gray-500 dark:text-gray-400">{{ $entry->ChangeDate }}
                                            </td>
                                        @else
                                            <td class="py-2 pr-4 text-gray-700 dark:text-gray-300">
                                                {{ number_format($entry->Silk_Offset) }}</td>
                                            <td
                                                class="py-2 pr-4 font-medium
                                                {{ $entry->BuyQuantity > 0 ? 'text-emerald-600 dark:text-emerald-400' : ($entry->BuyQuantity < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white') }}">
                                                {{ number_format($entry->BuyQuantity) }}
                                            </td>
                                            <td class="py-2 pr-4 text-gray-700 dark:text-gray-300">
                                                {{ number_format($entry->Silk_Remain) }}</td>
                                            <td class="py-2 text-gray-500 dark:text-gray-400">{{ $entry->AuthDate }}
                                            </td>
                                        @endif
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>

                    <div class="mt-6">
                        {{ $history->links() }}
                    </div>
                @endif
            </div>

        </div>
    </div>
</x-app-layout>
