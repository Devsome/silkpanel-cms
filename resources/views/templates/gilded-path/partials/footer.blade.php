...
<footer class="mt-auto border-t bg-neutral-950" style="border-color: rgba(120,90,0,0.5);">
    <div class="mx-auto max-w-[1600px] px-4 md:px-8 py-12">
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-10">
            ...
            <div class="flex flex-col gap-6">
                <div class="text-yellow-600 font-bold font-headline tracking-widest uppercase text-xl">
                ...
                <div class="flex flex-wrap gap-x-8 gap-y-2">
                    <a href="{{ route('news.index') }}"
                        class="text-neutral-600 hover:text-yellow-500 font-headline italic text-xs transition-colors duration-500">
                        {{ __('navigation.news') }}
                    </a>
                    ...
                    <div class="flex gap-4">
                        @if ($socialDiscord)
                            <a href="{{ e($socialDiscord) }}" target="_blank" rel="noopener noreferrer"
                                class="text-yellow-900 hover:text-yellow-500 transition-all" title="Discord">
                        ...
                        @if ($socialFacebook)
                            <a href="{{ e($socialFacebook) }}" target="_blank" rel="noopener noreferrer"
                                class="text-yellow-900 hover:text-yellow-500 transition-all" title="Facebook">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        @if ($socialTwitter)
                            <a href="{{ e($socialTwitter) }}" target="_blank" rel="noopener noreferrer"
                                class="text-yellow-900 hover:text-yellow-500 transition-all" title="Twitter / X">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        @if ($socialInstagram)
                            <a href="{{ e($socialInstagram) }}" target="_blank" rel="noopener noreferrer"
                                class="text-yellow-900 hover:text-yellow-500 transition-all" title="Instagram">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                ...
                {{-- Copyright --}}
                <p class="text-neutral-600 font-headline italic text-xs">
            ...
            @if (is_array($partners) && count($partners) > 0)
                <div class="md:text-right">
                    <h3 class="mb-4 text-xs font-headline font-bold uppercase tracking-widest text-yellow-700">
                        {{ __('footer.partners') }}
                    </h3>
                    <div class="flex flex-wrap md:justify-end items-center gap-6">
                        @foreach ($partners as $partner)
                            @if (!empty($partner['name']))
                                <a href="{{ e($partner['url'] ?? '#') }}" target="_blank" rel="noopener noreferrer"
                                    class="flex items-center gap-2 text-neutral-500 hover:text-yellow-500 transition duration-500"
                                    ...
                                    @endif
                                    <span class="text-sm font-medium font-headline">{{ e($partner['name']) }}</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
