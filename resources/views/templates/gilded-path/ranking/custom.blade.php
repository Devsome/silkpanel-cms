            ...
            <div class="mb-8 flex items-center gap-2 gp-card p-1.5">
                ...
                @forelse ($customRankings as $customRanking)
                    <a href="{{ route('ranking.custom', ['key' => $customRanking['key']]) }}"
                        class="flex-1 sm:flex-none px-5 py-2 text-center text-sm font-bold font-headline uppercase tracking-widest transition
                            {{ $activeCustomKey === ($customRanking['key'] ?? null) ? 'gp-gold-btn shadow-lg' : 'gp-text-on-surface-variant hover:text-yellow-400 hover:bg-yellow-900/10' }}">
                        <span class="inline-flex items-center gap-1.5">{{ e($customRanking['title']) }}</span>
                    </a>
                @empty
                    <a href="{{ route('ranking.custom') }}"
                        class="flex-1 sm:flex-none px-5 py-2 text-center text-sm font-bold font-headline uppercase tracking-widest transition gp-text-on-surface-variant hover:text-yellow-400 hover:bg-yellow-900/10">
