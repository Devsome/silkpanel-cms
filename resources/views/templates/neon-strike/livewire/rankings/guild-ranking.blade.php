    ...
    <div class="mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="relative w-full sm:w-64">
            <div class="flex items-center border border-zinc-700 bg-zinc-950 overflow-hidden">
                <svg class="ml-3 w-4 h-4 shrink-0 text-zinc-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @foreach ($columns as $col)
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-zinc-300">
                                    ...
                                    @elseif ($col['column'] === 'FoundationDate' && $value !== '—')
                                        <span
                                            class="font-mono text-xs text-zinc-500">{{ \Carbon\Carbon::parse($value)->format('d M Y') }}</span>
                                    @else
                                        {{ e((string) $value) }}
                                    @endif
                                </td>
