            ...
            <div class="bg-zinc-900 border border-violet-500/20 p-8 shadow-[0_0_50px_rgba(139,92,246,0.1)]">
                <div class="mb-6">
                    <p class="text-xs font-mono uppercase tracking-[0.3em] text-violet-400/70 mb-2">
                        {{ __('auth/confirm-password.title') }}</p>
                    <div class="h-px bg-linear-to-r from-violet-500/50 to-transparent mb-4"></div>
                    <p class="text-sm text-zinc-500">{{ __('auth/confirm-password.description') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.confirm') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="password"
                            class="block text-xs font-mono uppercase tracking-wider text-zinc-500 mb-1.5">{{ __('auth/confirm-password.form.password') }}</label>
