...
<footer class="mt-auto" style="border-top: 1px solid rgba(34,211,238,0.08);">
    ...
    <div class="mx-auto max-w-[1600px] px-4 md:px-8 py-12">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-10">
            ...
            <div class="md:col-span-4">
                ...
                <p class="text-xs leading-relaxed ag-text-muted max-w-xs">
                    @settings('site_description', 'A premium Silkroad Online private server experience. Join thousands
                    of players in the ultimate MMORPG journey.')
                </p>
            ...
            <div class="md:col-span-2">
                <p class="ag-font-display text-xs font-semibold tracking-widest uppercase ag-text-primary mb-4">
                    {{ __('footer.navigation') }}</p>
                <div class="space-y-2.5">
                    <a href="{{ route('index') }}"
                        class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.index') }}</a>
                    <a href="{{ route('news.index') }}"
                        class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.news') }}</a>
                    <a href="{{ route('downloads.index') }}"
                        class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.downloads') }}</a>
                    <a href="{{ route('ranking.characters') }}"
                        class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.rankings') }}</a>
                    @if ($tosEnabled)
                        <a href="{{ route('terms') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('footer.terms') }}</a>
                    @endif
                </div>
            </div>
                ...
                <div class="space-y-2.5">
                    @auth
                        <a href="{{ route('dashboard') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.dashboard') }}</a>
                        <a href="{{ route('profile.edit') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.profile') }}</a>
                        <a href="{{ route('donate.index') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.donation') }}</a>
                    @else
                        <a href="{{ route('login') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.login') }}</a>
                        <a href="{{ route('register') }}"
                            class="block text-xs ag-text-muted hover:ag-text-surface transition-colors">{{ __('navigation.register') }}</a>
                    @endauth
                </div>
            </div>
                    ...
                    <div class="space-y-2.5">
                        @if ($contactEmail)
                            <a href="mailto:{{ e($contactEmail) }}"
                                class="flex items-center gap-2 text-xs ag-text-muted hover:ag-text-surface transition-colors">
        ...
        @if (!empty($partners))
            <div class="mt-10 pt-8 border-t ag-divider">
                <p class="ag-font-display text-xs font-semibold tracking-widest uppercase ag-text-muted mb-4">
                    {{ __('footer.partners') }}</p>
                            ...
                            <a href="{{ e($partner['url']) }}" target="_blank" rel="noopener noreferrer"
                                ...
                                @if (!empty($partner['image']))
                                    <img src="{{ asset('storage/' . $partner['image']) }}"
                                        alt="{{ e($partner['name'] ?? '') }}" class="h-6 w-auto">
                                @else
    ...
    <div style="border-top: 1px solid var(--ag-outline-variant);">
        <div
            class="mx-auto max-w-[1600px] px-4 md:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p class="text-xs ag-text-muted">
                &copy; {{ date('Y') }} @settings('site_title', 'SilkPanel CMS').
                {{ __('footer.all_rights_reserved') }}
            </p>
            <p class="text-xs ag-text-muted">
                Powered by <span class="ag-text-primary font-semibold"><a href="https://devso.me" target="_blank"
                        rel="noopener noreferrer">SilkPanel CMS</a></span>
            </p>
        </div>
    </div>
</footer>
