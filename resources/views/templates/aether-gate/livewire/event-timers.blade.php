<div>
    @if (count($timers) > 0)
        <div class="ag-card p-5">
            <p class="ag-section-eyebrow mb-4">{{ __('index.event_timers') }}</p>
            <div class="space-y-2">
                @foreach ($timers as $timer)
                    <div class="flex items-center justify-between gap-3 p-3 ag-card-low">
                        <div class="flex items-center gap-3 min-w-0">
                            @if ($timer['image'])
                                <img src="{{ asset('storage/' . $timer['image']) }}" alt="{{ e($timer['name']) }}"
                                    class="h-[40px] w-[40px] object-cover shrink-0">
                            @elseif ($timer['icon'])
                                <x-dynamic-component :component="'heroicon-o-' . $timer['icon']"
                                    class="h-5 w-5 ag-text-primary shrink-0" />
                            @else
                                <x-heroicon-o-clock class="h-5 w-5 ag-text-primary shrink-0" />
                            @endif
                            <span class="text-xs ag-text-muted truncate">{{ e($timer['name']) }}</span>
                        </div>

                        <div class="shrink-0">
                            @if ($timer['type'] === 'static')
                                <span class="text-sm ag-text-muted">{{ e($timer['time']) }}</span>
                            @elseif ($timer['next_event'])
                                <div x-data="eventCountdown('{{ $timer['next_event']->toIso8601String() }}')"
                                    class="flex items-center gap-0.5 ag-font-mono text-sm font-bold tabular-nums ag-text-primary">
                                    <template x-if="parseInt(days) > 0">
                                        <span>
                                            <span x-text="days">00</span><span
                                                class="ag-text-muted text-xs">{{ __('event-timers.d') }}</span>
                                        </span>
                                    </template>
