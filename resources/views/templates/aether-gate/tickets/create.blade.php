    ...
    <section class="py-10">
        <div class="mx-auto max-w-3xl px-4 md:px-8">
            ...
            <div class="ag-card p-6 md:p-8">
                <p class="ag-section-eyebrow">{{ __('ticket::ticket.page_create_subtitle') }}</p>
                <h1 class="ag-section-title mt-2">{{ __('ticket::ticket.page_create_title') }}</h1>
                ...
                <form method="POST" action="{{ route('tickets.store') }}" enctype="multipart/form-data"
                    ...
                    <div>
                        <label for="title"
                            class="mb-1 block text-xs ag-font-display uppercase tracking-wider ag-text-muted">
                            {{ __('ticket::ticket.field_title') }} <span class="text-red-400">*</span>
                        </label>
                        <input type="text" id="title" name="title" value="{{ old('title') }}" required
                            class="ag-input w-full rounded px-3 py-2 text-sm @error('title') border-red-500 @enderror">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select id="category_id" name="category_id" required
                            class="ag-input w-full rounded px-3 py-2 text-sm @error('category_id') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <select id="priority" name="priority" required
                            class="ag-input w-full rounded px-3 py-2 text-sm @error('priority') border-red-500 @enderror">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <a href="{{ route('tickets.index') }}"
                            class="ag-btn-secondary px-4 py-2 text-xs ag-font-display font-bold uppercase tracking-wider">
