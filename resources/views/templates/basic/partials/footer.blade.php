    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-2">
                @if (\App\Helpers\SettingHelper::get('logo'))
                    <img src="{{ asset('storage/' . \App\Helpers\SettingHelper::get('logo')) }}"
                        alt="@settings('site_title', 'SilkPanel')" class="h-6 w-auto opacity-60" />
                @endif
                <span class="text-sm text-gray-500 dark:text-gray-400">
                    &copy; {{ date('Y') }} @settings('site_title', 'SilkPanel'). All rights reserved.
                </span>
            </div>
            <div class="flex items-center gap-4">
                ...
                @if (\App\Helpers\SettingHelper::get('social_discord'))
                    <a href="{{ \App\Helpers\SettingHelper::get('social_discord') }}" target="_blank"
